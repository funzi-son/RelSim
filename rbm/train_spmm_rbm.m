function [Wg Wb visBb hidB ms sigs] = train_spmm_rbm(conf,g_dat,b_dat)
...
for i=1:conf.eNum
    ...
    for j=1:conf.bNum
       ...
       if conf.lambda >0
           ...
           Wg = Wg + lr*conf.lambda*(repmat(pppp,visGNum,1).*(visPg'*((hidP.^2).*exp(-hidI))/sNum));
           Wb = Wb + lr*conf.lambda*(repmat(pppp,visBNum,1).*(visPb'*((hidP.^2).*exp(-hidI))/sNum));
           %u_diff = 
